...
async function seedUserRole (){
  try {
    ...
    if (user && user.isPasswordMatch('9scPILak7pTbpRw') && userRole){
    ...
    if (admin && admin.isPasswordMatch('pQAZ9lfvk27PEff') && adminRole){
      let count = await UserRole.countDocuments({
        userId: admin.id,
        roleId: adminRole.id,
        isActive: true, 
...
async function seedUser () {
  try {
    ...
    if (!user || !user.isPasswordMatch('9scPILak7pTbpRw') ) {
      let user = new User({
    ...
    if (!admin || !admin.isPasswordMatch('pQAZ9lfvk27PEff') ) {
      let admin = new User({
